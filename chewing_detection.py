...
4. **Pause/Play logic** – If chewing is absent for 5 seconds we send a pause
   command.  When chewing resumes we send play.
5. **YouTube control** – On macOS the script simulates a spacebar key press via
   :mod:`pyautogui`.  For iPadOS we provide a stub where integration with a
   Safari extension or custom URL scheme would occur.
6. **UI** – The live video feed is displayed with OpenCV, overlaying landmarks
   and status text.  This works out of the box on macOS.  For iPadOS a GUI would
   require a different framework, so this portion is left as a placeholder.
